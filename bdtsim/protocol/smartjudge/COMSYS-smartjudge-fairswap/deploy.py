...
mediator_address = deploy_contract(w3, bytecode, abi)
print("Deployed mediator smart contract to: {0}".format(mediator_address))
...
f.write(mediator_address)
f.close()

#deploy atomic swap verifier contract
bytecode, abi = compile_source_file('.', 'atomicswap.sol')

address = deploy_contract(w3, bytecode, abi, params=mediator_address)
...
#deploy fairswap verifier contract
bytecode, abi = compile_source_file('.', 'fairswap.sol')

address = deploy_contract(w3, bytecode, abi, params=mediator_address)
print("Deployed fairswap verifier to: {0}".format( address))

f = open("fairswap-verifier.abi", "w")
json.dump(abi, f)
f.close()

f = open("fairswap-verifier.addr", "w")
f.write(address)
f.close()