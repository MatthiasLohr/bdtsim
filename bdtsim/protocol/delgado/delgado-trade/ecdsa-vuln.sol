...
contract Delgado {
   ...
   */
   ...
   uint _r;
    ...
    function testVulnerability() public pure returns (bool,uint){
        ...
        return verifySellerPrivateKey(r,s1,s2,z1,z2);
    }
    
    function verifySellerPrivateKey(uint r,uint s1, uint s2, uint z1, uint z2) private pure returns (bool,uint){
        ...
        uint k = (z1 - z2) * longpow(s1 - s2,n-2,n) % n;
        
        uint x1 = (s1*k-z1) *longpow(r,n-2,n) % n;
        uint x2 = (s2*k-z2) *longpow(r,n-2,n) % n;
        return (x1 == x2,k);

    }
    function longpow(uint x, uint y, uint z) internal pure returns (uint r) {
    uint number = 1;
    while (y!= 0)
    {
        if ((y & 1)==0){
    ...
    function BuyerRequestKey(uint z1, uint s1, uint r ) public payable{
        _z1 = z1;
        _s1 = s1;
        _r = r;
    }
    function SellerRetrieveEther(uint z2,uint s2) public {
    }
}
